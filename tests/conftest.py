...
def server_url(tmp_path_factory: pytest.TempPathFactory) -> Generator[str, None, None]:
    ...
    """
    # Allow overriding via env var for testing against running dev server
    if existing_url := os.environ.get("TEST_SERVER_URL"):
        yield existing_url
        return
    ...
    port = _get_free_port()
    # Note: Adjust module path if needed. Assuming apps.web.main is correct relative to REPO_ROOT
