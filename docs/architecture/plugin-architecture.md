...
## Core vs Plugins
The core package (`arbolab`) provides domain models, configuration models, layouts, stores, persistence, and the plugin runtime. It must never depend on device packages at compile time.
...
- Each plugin exposes a `register(registry)` function that registers loaders and processors with the core registry.
...
## Extension Points
Core extension points allow plugins to register ingestion and reporting providers. Plugins receive a shared context object that exposes core layouts, stores, and logging.
