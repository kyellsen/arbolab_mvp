...
- Core package: `arbolab` (domain models, configuration, layouts, stores, persistence, plugin runtime, `Lab` API).
...
- Core code is organized by responsibility (`layouts/`, `stores/`, `models/`, `plugins/`, `schemas/`, `services/`).
- Device plugin code lives under `src/arbolab_plugins/<device>/`.
- Device plugin code is organized by responsibility (for example `io/`, `models/`, `schemas/`, `services/`).
...
- `docs/architecture/services-and-runtime.md` for configuration, runtime components, and service wiring.
- `docs/architecture/examples.md` for examples layout and fixture conventions.

## Entry Point
`Lab` is the integration point for configuration, storage layout, stores, services, and plugin lifecycle. API constraints are defined in `docs/specs/api.md`.
