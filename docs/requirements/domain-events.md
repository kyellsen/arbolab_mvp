...
## Purpose
Use a list of domain events as a lightweight backbone for requirements and modeling.
Events help clarify what happens, when it is considered "done", and what data is relevant.
...
- "Done" condition (when is the event true?): the template package exists and is structurally valid (manifest + empty tables)

### MetadataPackageImported
- Event name: MetadataPackageImported
- Trigger: user imports a filled metadata package under `input_root/metadata`
